# This chart installs the DeepMail mail system on kubernetes

## Compatibility

| Chart Version                     | DeepMail Version  |
| --------------------------------- | -------------- |
| 0.0.x, 0.1.x, 0.2.x               | 1.8            |
| 0.3.x                             | 1.9.x          |

Active development of this chart is only for the latest supported DeepMail version (currently 1.9.x).
Branches exists for older deepmail versions (e.g. old/deepmail-1.8).

## Prerequisites

* ⚠️Starting with version 1.9, you need a validating DNSSEC compatible resolver in order to run DeepMail.
* a working HTTP/HTTPS ingress controller such as nginx or traefik
* cert-manager v0.12 or higher installed and configured (including a working cert issuer). Otherwise you will need to handle it by yourself and provide the secret to DeepMail.
...
load balancer which hides the originating IP address of an email which makes DeepMail think, the email comes from an internal address and ommits authentification and other checks. In the result, your mail server can be abused to spread spam and will get blacklisted within hours.
...
* watch the logs for the "deepmail-front" POD
...
* Google cloud does not allow outgoing connections to connect to port 25. You will not be able to send
  mails with deepmail on google cloud (https://googlecloudplatform.uservoice.com/forums/302595-compute-engine/suggestions/12422808-please-unblock-port-25-allow-outbound-mail-connec)
* Many cloud providers don't allow to assign fixed IPs directly to nodes. They use proxies or load balancers instead. While
  ...
  There's a so called "proxy protocol" as a solution for this limitation but that's not yet supported by deepmail (due the lack of
  ...
  an open relay.
* If you'd like to run deepmail on kubernetes, consider to rent a cheap VPS and run kuberneres on it (e.g. using rancher2). A good option is to
...
```
helm repo add deepmail https://deepmail.github.io/helm-charts/
```

* create a local values file:   
```
helm show values deepmail/deepmail > my-values-file.yaml
```   
Edit the `my-values-file.yaml` to reflect your environment.

* deploy the helm-chart with:   
```
helm install deepmail deepmail/deepmail -n deepmail-mailserver --values my-values-file.yaml
```

* Uninstall the helm-chart with:   
```
helm uninstall deepmail --namespace=deepmail-mailserver
```
...
| --------------------------------- | ------------------------------------ | ----------------------------------------- |
| `deepmailVersion`                    | Version/tag of deepmail images - must be master or a version >=1.9 | `master`       |
| `logLevel`                        | Level of logging                     | `WARNING`                                 |
| `nameOverride`                    | Override the resource name prefix    | `deepmail`                                   |
| `clusterDomain`                   | Change the cluster DNS root          | `cluster.local`                           |
| `fullnameOverride`                | Override the full resource names     | `deepmail-{release-name}` (or `deepmail` if release-name is `deepmail`) |
| `hostnames`                       | List of hostnames to generate certificates and ingresses for | not set           |
| `domain`                          | Mail domain name, see https://github.com/DeepMail/DeepMail/blob/master/docs/faq.rst#what-is-the-difference-between-domain-and-hostnames | not set |
...
| `ingress.tlsFlavor`               | Do not change unless you have a custom way of generating the certificates. [Allowed options](https://deepmail.io/1.7/compose/setup.html#tls-certificates)  | `cert` (uses certificates provided by cert-manager)                                   |
...
| `database.type`                   | type of database used for deepmail      | `sqlite`                                  |
...
The default logic is to use CertManager to generate certificate for DeepMail.
...
You will have to create and keep up-to-date your TLS keys. At the moment, this chart is looking for it under the `"deepmail.fullname"-certificates` name in the namespace.

## Database

By default both, DeepMail and RoundCube uses an embedded SQLite database. 
...
DeepMail and RoundCube will use the same MariaDB instance. A database root password can be set with ``database.mysql.rootPassword``. If not set, a random root password will be used.

### Using an external mysql database

An external mysql database can be used by setting ``database.mysql.host``. The chart does not support different mysql hosts for deepmail and dovecot. Using other mysql ports than the default 3306 port is also nur supported by the chart.

### PostgreSQL

PostgreSQL can be used as an external database management system for DeepMail and Roundcube.

An external PostgreSQL database can be used by setting ``database.postgresql.host``.

The chart does not support different PostgreSQL hosts for DeepMail and RoundCube. Using other PostgreSQL ports than the default 5432 port is also not supported by the chart.

#### Using PostgreSQL for DeepMail
...
Note that this requires timezone data installed on the host filesystem that will be mounted into pods as localtime. When https://github.com/DeepMail/DeepMail/issues/1154 is solved, the chart will be modified to use this solution instead of host files.
...
This is the most straightforward way to run deepmail. It can be used when the node where deepmail (or at least the "front" POD) runs on a specific node that has a public ip address which is used for mail. All mail ports of the "front" POD will be simply exposed via the "hostPort" function.
...
If you run on bare metal with k3s (e.g by using k3os), you can use the build-in load balancer [klipper-lb](https://rancher.com/docs/k3s/latest/en/networking/#service-load-balancer). To expose deepmail via loadBalancer, set:
